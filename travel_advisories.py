...
# ---------------------------------------------------------------------------
# State Dept advisory URL builder
# ---------------------------------------------------------------------------
# State's advisory pages use the pattern:
#   https://travel.state.gov/en/international-travel/travel-advisories/<slug>.html
# where <slug> is the lowercased country name with non-alphanumerics collapsed
# to hyphens (e.g. "Bangladesh" -> bangladesh, "Solomon Islands" -> solomon-islands).
# Most names build correctly straight from the feed; the override map below pins
# the few whose State slug differs from a plain slugify of the feed's name.
#
# To add one: open
#   https://travel.state.gov/en/international-travel/travel-advisories/destination.html
# pick the country in "Select your destination", press Go, and copy the slug
# (the part of the resulting URL before ".html"). Key it by the EXACT name the
# dashboard shows for that country (left-panel label).
_ADVISORY_BASE = 'https://travel.state.gov/en/international-travel/travel-advisories/'

_ADVISORY_SLUG_OVERRIDES = {
    'Kingdom of Denmark': 'denmark',
}

def _advisory_slug(name):
    s = unicodedata.normalize('NFKD', name).encode('ascii', 'ignore').decode('ascii').lower()
    return re.sub(r'[^a-z0-9]+', '-', s).strip('-')

def _advisory_link(name):
    if name in _ADVISORY_SLUG_OVERRIDES:
        return _ADVISORY_BASE + _ADVISORY_SLUG_OVERRIDES[name] + '.html'
    return _ADVISORY_BASE + _advisory_slug(name) + '.html'
